---
slug: "fonte-paragrafo-estilos"
modulo: "Módulo 1 — Primeiros Passos no Word"
titulo: "Fonte, Parágrafo & Estilos de Texto"
subtitulo: "Formatação precisa com fonte e parágrafo, e padronização com estilos"
descricao: "Formatar textos com precisão usando fonte e parágrafo, e padronizar documentos inteiros automaticamente com estilos de texto."
ordem: 2
proximosPassos:
  - titulo: "Tabulações, Listas & Tabelas"
    descricao: "Organize informações com tabulações, listas multinível e tabelas"
  - titulo: "Avaliação AV1"
    descricao: "Prepare-se para a primeira avaliação prática"
quiz:
  - pergunta: "Qual é o erro mais comum ao espaçar parágrafos?"
    opcoes: ["Usar Tab no início", "Pressionar Enter duas vezes", "Usar espaçamento de 1,5", "Aplicar estilo Normal"]
    correta: 1
    explicacao: "Pressionar Enter duas vezes cria espaçamento inconsistente. O correto é configurar 'Espaçamento Depois' nas opções de Parágrafo."
    explicacaoErrada: "O erro mais comum é pressionar Enter duas vezes. Use o campo 'Espaçamento Depois' em Parágrafo para manter consistência."
  - pergunta: "Qual atalho configura o alinhamento Justificado (padrão ABNT)?"
    opcoes: ["Ctrl + E", "Ctrl + J", "Ctrl + Q", "Ctrl + G"]
    correta: 1
    explicacao: "Ctrl + J aplica o alinhamento Justificado, que é o padrão ABNT para o corpo do texto."
    explicacaoErrada: "O atalho para Justificado é Ctrl + J. Os outros são: Ctrl + E (centralizado), Ctrl + Q (esquerda), Ctrl + G (direita)."
  - pergunta: "Por que é importante usar estilos (Título 1, Título 2, Normal) no documento?"
    opcoes: ["Para deixar o texto mais bonito", "Para que o sumário automático funcione e padronizar o documento", "Para mudar a cor do texto", "Para adicionar bordas"]
    correta: 1
    explicacao: "Estilos permitem padronização automática e são obrigatórios para gerar o sumário automático do Word."
    explicacaoErrada: "Estilos vão muito além da estética: permitem padronizar todo o documento de uma vez e são essenciais para o sumário automático."
  - pergunta: "Qual o espaçamento entre linhas padrão ABNT para o corpo do texto?"
    opcoes: ["Simples (1,0)", "1,5 linhas", "Duplo (2,0)", "2,5 linhas"]
    correta: 1
    explicacao: "O padrão ABNT é espaçamento de 1,5 linhas para o corpo do texto. Atalho: Ctrl + 5."
    explicacaoErrada: "O espaçamento ABNT é 1,5 linhas (Ctrl + 5). Simples é para legendas e duplo para documentos de revisão."
---

## Formatação de Fonte

Tudo fica em **Página Inicial → grupo Fonte**.
Para abrir todas as opções de uma vez: `Ctrl + D`.

### Tipos de fonte

| Tipo                    | Exemplos                  | Quando usar                           |
| ----------------------- | ------------------------- | ------------------------------------- |
| Serif (com serifa)      | Times New Roman, Garamond | Documentos acadêmicos e formais       |
| Sans-serif (sem serifa) | Arial, Calibri            | Documentos modernos e digitais        |
| Monoespaçada            | Courier New               | Código de programação e texto técnico |

> **Regra:** Use no máximo 2 famílias de fonte por documento.

### Atalhos de caractere

| Atalho          | Efeito            | Quando usar                       |
| --------------- | ----------------- | --------------------------------- |
| `Ctrl + N`      | **Negrito**       | Termos importantes, destaques     |
| `Ctrl + I`      | _Itálico_         | Títulos de obras, estrangeirismos |
| `Ctrl + S`      | Sublinhado        | Usar com moderação                |
| `Ctrl + D`      | Janela Fonte      | Todas as opções avançadas         |
| `Ctrl + Espaço` | Limpar formatação | Volta ao estilo Normal            |
| `Ctrl + ]`      | Aumentar fonte    | Sem usar o mouse                  |
| `Ctrl + [`      | Diminuir fonte    | Sem usar o mouse                  |

### Limpar formatação de texto colado da internet

Quando você copia texto de um site (`Ctrl + C`) e cola no Word (`Ctrl + V`), a formatação vem toda bagunçada — cores, links, fontes diferentes. Existem duas soluções:

1. **Colar sem formatação:** Página Inicial → setinha abaixo de Colar → **Manter Somente Texto** (terceiro ícone)
2. **Limpar depois:** Selecione o texto colado → `Ctrl + Espaço` — remove toda a formatação e links

> **Dica do professor:** Na hora de colar, o botão Colar tem três opções: manter formatação original, mesclar formatação, ou manter somente o texto. Use a terceira quando colar da internet.

### Pincel de Formatação

- Copia a formatação de um trecho e aplica em outro
- Ícone de pincel no grupo **Área de Transferência**
- **1 clique** = aplica em um trecho
- **Duplo clique** = modo contínuo (vários trechos)

---

## Formatação de Parágrafo

Acesse em **Página Inicial → grupo Parágrafo** ou botão direito → Parágrafo.

### O erro mais comum

**Errado:** pressionar Enter duas vezes entre parágrafos
**Certo:** configurar o campo **"Espaçamento Depois"** em Parágrafo

> **Nunca use Enter duplo para espaçar parágrafos.** Use o campo "Espaçamento Depois" — isso garante consistência e permite que o sumário automático funcione.

### Alinhamento

| Atalho     | Tipo         | Quando usar                     |
| ---------- | ------------ | ------------------------------- |
| `Ctrl + Q` | Esquerda     | Textos informais, e-mails       |
| `Ctrl + E` | Centralizado | Títulos, capas                  |
| `Ctrl + G` | Direita      | Datas, assinaturas              |
| `Ctrl + J` | Justificado  | Padrão ABNT para corpo do texto |

### Espaçamento entre linhas

| Atalho     | Espaçamento   | Uso                                 |
| ---------- | ------------- | ----------------------------------- |
| `Ctrl + 1` | Simples (1,0) | Rodapés, legendas                   |
| `Ctrl + 5` | 1,5 linhas    | **Padrão ABNT** para corpo do texto |
| `Ctrl + 2` | Duplo (2,0)   | Documentos para revisão             |

### Configuração ABNT completa — passo a passo

1. Fonte: **Arial** ou **Times New Roman**, tamanho **12**
2. Alinhamento: **Justificado** — `Ctrl + J`
3. Espaçamento entre linhas: **1,5** — `Ctrl + 5`
4. Espaçamento Depois: **0pt** — Parágrafo → Espaçamento → Depois: 0
5. Recuo de primeira linha: **1,25cm** — Parágrafo → Recuos Especiais → Primeira linha

---

## Estilos de Texto

Um **estilo** é um conjunto de formatações com um nome. Ao modificar o estilo, todos os trechos com ele atualizam automaticamente de uma vez.

### Hierarquia de estilos

| Estilo       | Uso                               |
| ------------ | --------------------------------- |
| **Título 1** | Capítulos principais              |
| **Título 2** | Subcapítulos                      |
| **Título 3** | Subsubcapítulos                   |
| **Normal**   | Corpo do texto                    |
| **Citação**  | Citações longas (ABNT: recuo 4cm) |

> **Fundamental:** Use sempre Título 1, 2 e 3 para estruturar o documento. O **sumário automático** (Aula 06) só funciona com eles.

### Por que usar estilos

| Sem estilos                                          | Com estilos                                 |
| ---------------------------------------------------- | ------------------------------------------- |
| 80 títulos para mudar de cor: 30 minutos de trabalho | Modificar o estilo uma vez: 10 segundos     |
| Inconsistências visuais em documentos longos         | Padronização automática em todo o documento |
| Sumário automático não funciona                      | Sumário gerado em 1 clique                  |

### Como modificar um estilo

1. Botão direito no estilo na galeria → **Modificar**
2. Altere: fonte, tamanho, cor, espaçamento
3. Clique em **OK** — todos os trechos com aquele estilo atualizam

### Como criar um estilo novo

1. Clique na setinha para baixo na galeria de estilos → **Criar um Estilo**
2. Dê um nome ao estilo (ex: "Frase Importante")
3. Clique em **Modificar** para configurar fonte, tamanho, cor, recuo, espaçamento
4. Em **"Estilo do parágrafo seguinte"**, escolha para qual estilo o Word deve mudar ao pressionar Enter (ex: Normal)
5. Clique em **OK** — o novo estilo aparece na galeria

> **Dica do professor:** Configure o "estilo do parágrafo seguinte" como Normal nos seus títulos. Assim, ao dar Enter após um título, o Word já muda automaticamente para o estilo de corpo de texto — você não precisa trocar manualmente.

### Por que usar estilos é mais inteligente que formatação manual

Imagine que você formatou 80 títulos manualmente — mudou fonte, tamanho, cor, um por um. Agora o professor pede para mudar a cor de todos os títulos. Com formatação manual, você teria que mudar cada um dos 80. **Com estilos, basta modificar o estilo uma vez e todos os 80 atualizam em 10 segundos.** Além disso, sem estilos o sumário automático (Aula 06) simplesmente não funciona.

---

## Checklist da Aula

- [ ] Configurei um parágrafo completo com as normas ABNT
- [ ] Testei negrito (`Ctrl+N`), itálico (`Ctrl+I`) e limpeza de formatação (`Ctrl+Espaço`)
- [ ] Usei o Pincel de Formatação para copiar aparência entre trechos
- [ ] Apliquei os estilos Título 1, Título 2 e Normal em um documento
- [ ] Modifiquei um estilo e vi todos os títulos atualizarem automaticamente

---

## Referências

- [Digitação e Formatação Básica (~21min)](https://www.youtube.com/watch?v=JvSVkrDbzf0)
- [Formatações Baseadas em Estilos (~13min)](https://www.youtube.com/watch?v=ELtk2wmrmQc)
