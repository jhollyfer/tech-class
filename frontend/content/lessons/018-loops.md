...
E mais limpo que `for (let i = 0; i < frutas.length; i++)` quando voce so precisa do valor de cada elemento.

### Exemplo pratico: soma de um array
...
> [!sucesso]
> **Regra pratica para escolher o loop:**
> - Sabe quantas vezes? Use `for`
> - Nao sabe quantas vezes? Use `while`
> - Precisa executar pelo menos uma vez? Use `do-while`
> - Percorrer um array? Use `for...of`
...
for (let i: number = 1; i <= 10; i++) {
  if (i === 7) break;          // para o loop no 7
  if (i % 2 === 0) continue;   // pula números pares
  console.log(i);               // 1, 3, 5
}
```

### Exemplo pratico: buscar em um array

O `break` e muito util para parar de procurar quando encontrar o que precisa:
...
if (!encontrado) {
  console.log(`${busca} não encontrado`);
}
```

### Exemplo pratico: filtrar com continue

O `continue` e util para pular elementos que nao interessam:
