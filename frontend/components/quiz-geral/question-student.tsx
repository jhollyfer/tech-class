...
export function QuestionStudent({
  ...
  const timer = useQuizTimer(60, isActive);
  const autoSubmittedRef = useRef(false);

  // Sound plays only when: timer ≤15s AND student hasn't selected anything AND question is active
  useUrgencySound(
    timer.secondsLeft <= 15 && timer.secondsLeft > 0 && isActive && selectedOption === null,
    timer.secondsLeft
  );
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-3">
          {question.opcoes.map((opcao, i) => {
            ...
            const isLocked = hasAnswered || (timer.isCritical && selectedOption !== null);
